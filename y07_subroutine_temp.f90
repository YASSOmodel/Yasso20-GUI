...
IMPLICIT NONE
    ...
    REAL,DIMENSION(12),INTENT(IN) :: temp ! Monthly mean temperatures
    REAL,INTENT(IN) :: prec ! Annual precipitation
    ...
    climate = (/sum(temp)/12, prec, (maxval(temp)-minval(temp))/2/) 
    ...
    END DO
    A(4,4) = -ABS(theta(4))*tem*size_dep
    ...
    A(4,5) = 0.0
    A(5,5) = -ABS(theta(32))*tem ! no size effect in humus
    DO i = 1,4
